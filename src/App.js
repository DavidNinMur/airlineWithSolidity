...
export class App extends Component {
    ...
    async componentDidMount() {
        ...
        this.web3.currentProvider.on('accountsChanged', async function (accounts) {
            this.setState({
                account: accounts[0]
            }, () => {
                this.load();
                ...
                <div className="col-sm">
                    <Panel title="Available flights">
                        {this.state.flights.map((flight, i) => {
                            return <div key={i}>
                                <span>{flight.name} - cost: {this.toEther(flight.price)} ETH</span>
                ...
                <div className="col-sm">
                    <Panel title="Your flights">
                        {this.state.customerFlights.map((flight, i) => {
                            return <div key={i}>
                                <span>{flight.name} - Price : {this.toEther(flight.price)} ETH</span>
                            </div>
                        })}
                    </Panel>
                </div>
